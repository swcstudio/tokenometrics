...
<critical>You MUST have already loaded and processed: {project-root}/.osx/bmb/workflows/create-agent/workflow.yaml</critical>
<critical>Study YAML agent examples in: {project-root}/.osx/bmm/agents/ for patterns</critical>
...
<step n="-1" goal="Optional brainstorming for agent ideas" optional="true">
  ...
  <check if="user answered yes">
    <action>Invoke brainstorming workflow: {project-root}/.osx/core/workflows/brainstorming/workflow.yaml</action>
...
**Path Determination:**
  ...
  <check if="module agent selected">
    ...
    <note>Agent will be saved to: .osx/{{target_module}}/agents/</note>
  </check>

  <check if="standalone agent selected">
    <action>Explain this will be their personal agent, not tied to a module</action>
    <note>Agent will be saved to: .osx/agents/{{agent-name}}/</note>
    <note>All sidecar files will be in the same folder</note>
  </check>

<critical>Determine agent location:</critical>

- Module Agent → .osx/{{module}}/agents/{{agent-name}}.agent.yaml
- Standalone Agent → .osx/agents/{{agent-name}}/{{agent-name}}.agent.yaml
...
- trigger: [another capability]
  workflow: "{project-root}/.osx/SOURCE_MODULE/workflows/path/to/workflow.yaml"
  workflow-install: "{project-root}/.osx/THIS_MODULE/workflows/vendored/path/workflow.yaml"
...
<example type="yaml">
  agent:
    metadata:
      id: .osx/{{target_module}}/agents/{{agent_filename}}.md
