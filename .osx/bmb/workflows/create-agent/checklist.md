...
- [ ] `metadata.id` points to final compiled path: `.osx/{{module}}/agents/{{agent}}.md`
...
- [ ] Customize file (if created) located at `{project-root}/.osx/_cfg/agents/{{module}}-{{agent}}.customize.yaml`

## Build Verification

- [ ] Run compile to build `.md`: `npm run install:bmad` → "Compile Agents" (or `bmad install` → Compile)
- [ ] Confirm compiled file exists at `{project-root}/.osx/{{module}}/agents/{{agent}}.md`
