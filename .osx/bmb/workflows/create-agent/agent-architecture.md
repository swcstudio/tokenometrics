...
- `id` - Unique path identifier (e.g., ".osx/bmm/agents/analyst.md")
...
```xml
<critical-actions>
  <i>Load into memory {project-root}/.osx/{module}/config.yaml and set variables</i>
...
```xml
<module-integration>
  <module-path>{project-root}/.osx/{module-code}</module-path>
  <config-source>{module-path}/config.yaml</config-source>
  <workflows-path>{project-root}/.osx/{module-code}/workflows</workflows-path>
</module-integration>
```
...
```
Good: {project-root}/.osx/{module}/agents/
...
```xml
<!-- Full path -->
<item cmd="*create-prd" run-workflow="{project-root}/.osx/bmm/workflows/prd/workflow.yaml">
...
```xml
<item cmd="*validate" exec="{project-root}/.osx/core/tasks/validate-workflow.xml">
  Validate document
</item>
```
...
```xml
<item cmd="*standup"
   exec="{project-root}/.osx/bmm/tasks/daily-standup.xml"
...
<item cmd="*run" exec="{project-root}/.osx/module/task.md">
...
```xml
<critical-actions>
  <i>Load into memory {project-root}/.osx/{module}/config.yaml</i>
  <i>Remember the users name is {user_name}</i>
  <i>ALWAYS communicate in {communication_language}</i>
</critical-actions>
```
...
```xml
<agent id=".osx/{module}/agents/{name}.md"
       name="{Name}"
       title="{Title}"
       icon="{emoji}">
  <persona>...</persona>
