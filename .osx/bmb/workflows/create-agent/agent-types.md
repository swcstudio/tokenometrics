...
Live in their own dedicated directories under `.osx/agents/`:
...
Part of a module system under `.osx/{module}/agents/`:
...
**Location:** `.osx/agents/{agent-name}/`
...
**Location:** `.osx/agents/{agent-name}/` with sidecar directory
...
agent:
  ...
  persona:
    ...
    principles: ['...']
  critical_actions:
    - 'Load config from {project-root}/.osx/{module}/config.yaml'
  menu:
    - trigger: create-prd
...
<agent id=".osx/bmm/agents/pm.md" name="John" title="Product Manager" icon="📋">
  ...
  <critical-actions>
    <i>Load config from {project-root}/.osx/{module}/config.yaml</i>
  </critical-actions>
  <menu>
    <item cmd="*help">Show numbered menu</item>
    <item cmd="*create-prd" run-workflow="{project-root}/.osx/bmm/workflows/prd/workflow.yaml">Create PRD</item>
    <item cmd="*validate" exec="{project-root}/.osx/core/tasks/validate-workflow.xml">Validate document</item>
    <item cmd="*exit">Exit</item>
  </menu>
</agent>
```
