...
<parameter name="You MUST have already loaded and processed: {project-root}/.osx/bmb/workflows/convert-legacy/workflow.yaml</critical>
...
<action>Show user the target location: {project-root}/.osx/{{target_module}}/{{item_type}}/{{item_name}}</action>
<action>Note: Files will be created in .osx/ but all internal paths will reference {project-root}/.osx/ locations</action>
...
5. Handle dependencies and paths:
   ...
   - CRITICAL: All paths must use {project-root}/.osx/{{module}}/ NOT src/

<action>Generate the converted v6 agent YAML file (.agent.yaml)</action>
<action>Example path conversions:

- exec="{project-root}/.osx/{{target_module}}/tasks/task-name.md"
- run-workflow="{project-root}/.osx/{{target_module}}/workflows/workflow-name/workflow.yaml"
- data="{project-root}/.osx/{{target_module}}/data/data-file.yaml"
  </action>
  <action>Save to: .osx/{{target_module}}/agents/{{agent_name}}.agent.yaml (physical location)</action>
...
<invoke-workflow>
  workflow: {project-root}/.osx/bmb/workflows/create-agent/workflow.yaml
  inputs:
...
2. Convert template sections to instructions.md:
   ...
   - `elicit: true` → `<invoke-task halt="true">{project-root}/.osx/core/tasks/adv-elicit.xml</invoke-task>` tag
...
<invoke-workflow>
  workflow: {project-root}/.osx/bmb/workflows/create-workflow/workflow.yaml
  inputs:
...
<invoke-workflow>
  workflow: {project-root}/.osx/bmb/workflows/create-module/workflow.yaml
  inputs:
...
4. Handle special v4 patterns:
   - 1-9 elicitation menus → v6 <invoke-task halt="true">{project-root}/.osx/core/tasks/adv-elicit.xml</invoke-task>
   - Agent permissions → note in instructions
