...
- Ensures all paths use `{project-root}/.osx/` format
...
- Maps `elicit: true` flags to `<invoke-task halt="true">{project-root}/.osx/core/tasks/adv-elicit.xml</invoke-task>` tags
...
- **Solution**: Ensure all references use `{project-root}/.osx/` format
...
- Review the workflow creation guide at `/.osx/bmb/workflows/create-workflow/workflow-creation-guide.md`
- Check conversion mappings at `/.osx/bmb/data/v4-to-v6-mappings.yaml`
