...
<critical>You MUST have already loaded and processed: {project-root}/.osx/bmb/workflows/create-module/workflow.yaml</critical>
<critical>Study existing modules in: {project-root}/.osx/ for patterns</critical>
...
<check if="create">
  <action>Invoke module-brief workflow: {project-root}/.osx/bmb/workflows/module-brief/workflow.yaml</action>
...
2. **Module code** - Generate kebab-case from name following patterns:
   ...
   - Present suggested code and confirm it works for paths like .osx/{{code}}/agents/
...
# EXAMPLE Static path:
# data_path:
#   result: "{project-root}/.osx/{{module_code}}/data"
...
The module can be configured in `.osx/{{module_code}}/config.yaml`
